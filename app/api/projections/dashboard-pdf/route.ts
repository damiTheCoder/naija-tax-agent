...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    drawLineChart(doc, {
      ...
      series: [{ label: "Revenue", color: "#8fff00", values: revenueTrend.map((point) => point.revenue) }],
    });
    ...
    drawGroupedBarChart(doc, {
      ...
      series: [
        ...
        { label: "Net", color: "#8fff00", values: cashflow.map((point) => point.netCashflow) },
      ],
    });
    ...
    drawLineChart(doc, {
      ...
      series: [
        { label: "Revenue", color: "#8fff00", values: breakEven.map((point) => point.revenue) },
    ...
    drawLineChart(doc, {
      ...
      series: [
        ...
        { label: "Expected", color: "#8fff00", values: scenarioComparison.map((point) => point.expected) },
