...
const ProjectionsDashboard = dynamic(() => import("@/components/accounting/ProjectionsDashboard"), {
  loading: () => (
    <div className="rounded-2xl border border-gray-100 bg-white p-5 text-sm text-gray-500">
      Loading projections dashboard...
    </div>
  ),
  ssr: false,
});
...
export default function AccountingProjectionsPage() {
  ...
  const projectedEbitdaSixMonth = useMemo(
    () => expectedSixMonth.reduce((sum, point) => sum + point.ebitda, 0),
    [expectedSixMonth]
  );
  ...
  const handleDownloadDashboardPdf = async () => {
    ...
    try {
      ...
      globalThis.setTimeout(() => setIsDownloadingPdf(false), 600);
    }
  };
  ...
  return (
    <div className="projection-print-root space-y-5 lg:space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <Link href="/accounting/projections/modelling" className="mt-2 inline-flex text-sm font-medium text-[#446b00] hover:text-[#446b00]">
            Open Financial Modelling
          </Link>
        </div>
      ...
      <ProjectionsDashboard
        projectedRevenueAnnual={projectedRevenueAnnual}
        ...
        breakEvenMonthLabel={breakEven.monthLabel}
        breakEvenRevenue={breakEvenRevenue}
        projectionValidationIssues={projectionValidationIssues}
        projectionStatus={projectionStatus}
        actualSeriesLength={actualSeries.length}
        revenueData={revenueData}
        revenueStreams={breakdowns.revenueStreams}
        expenseStackData={expenseStackData}
        fixedExpenseTotal={breakdowns.fixedExpenseTotal}
        variableExpenseTotal={breakdowns.variableExpenseTotal}
        expenseCategories={breakdowns.expenseCategories}
        profitabilityData={profitabilityData}
        projectedEbitdaSixMonth={projectedEbitdaSixMonth}
        ...
        onResetAssumptions={() => setAssumptionOverrides({})}
        onUpdateAssumptionPercent={updateAssumptionPercent}
        onUpdateAssumptionNumber={updateAssumptionNumber}
      />
