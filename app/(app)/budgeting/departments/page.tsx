...
export default function DepartmentBudgetsPage() {
  ...
  const departmentRows = useMemo(() => {
    ...
    performanceRows.forEach((budget) => {
      budget.departmentRows.forEach((department) => {
        const current = map.get(department.department) || { budgeted: 0, actual: 0 };
        map.set(department.department, {
          budgeted: current.budgeted + department.budgeted,
          actual: current.actual + department.actual,
        });
      });
    });
    ...
    return Array.from(map.entries())
      .map(([department, values]) => {
        ...
        return { department, ...values, remaining, utilization };
      })
      .sort((a, b) => b.actual - a.actual);
  }, [performanceRows]);

  if (!isReady) {
    return <div className="rounded-2xl border border-gray-200 bg-white p-6 text-sm text-gray-500">Loading department budgets...</div>;
  }
  ...
  return (
    <div className="space-y-6 px-4 py-6 sm:px-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Allocate and monitor budget performance by department.</p>
      </div>
              ...
              {departmentRows.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-gray-500">
                    No department allocations yet. Add department budgets in Create/Edit Budget.
                  </td>
                </tr>
              ) : null}
