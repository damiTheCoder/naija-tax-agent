...
import {
  ...
  applySuggestion,
  applyAllSuggestions,
  applyQuickFillPreset,
  getApplicablePresets,
  getLawReference,
  AutoFillSuggestion,
  NIGERIAN_TAX_LAWS,
} from "@/lib/taxRules/autoFillEngine";
...
export default function HomePage() {
  ...
  // AI Statement Analysis state
  const [isAnalyzingStatement, setIsAnalyzingStatement] = useState(false);
  const [statementAnalysisResult, setStatementAnalysisResult] = useState<string | null>(null);
  ...
  // ===========================================================================
  // AUTO-FILL & SMART SUGGESTIONS
  // ===========================================================================

  // Get accounting data for auto-fill
  const getAccountingData = useCallback(() => {
    try {
      const statements = accountingEngine.generateStatements();
      return {
        revenue: statements.revenue || 0,
        expenses: (statements.costOfSales || 0) + (statements.operatingExpenses || 0),
        vatPaid: 0, // Could be extended to track VAT
      };
    } catch {
      return { revenue: 0, expenses: 0, vatPaid: 0 };
    }
  }, []);
  ...
  const refreshSuggestions = useCallback(() => {
    ...
    const suggestions = getAutoFillSuggestions(profile, inputs, accountingData);
    setAutoFillSuggestions(suggestions);
  }, [profile, inputs, getAccountingData]);

  // Refresh suggestions when step 2 is reached
  useEffect(() => {
    if (step === 2) {
      refreshSuggestions();
    }
  ...
  const handleApplySuggestion = (suggestion: AutoFillSuggestion) => {
    const newInputs = applySuggestion(inputs, suggestion);
    setInputs(newInputs);
    // Remove applied suggestion
    setAutoFillSuggestions(prev => prev.filter(s => s.field !== suggestion.field));
  };

  // Apply all suggestions
  const handleApplyAllSuggestions = () => {
    const newInputs = applyAllSuggestions(inputs, autoFillSuggestions);
    setInputs(newInputs);
  ...
  const handleImportFromAccounting = () => {
    const accountingData = getAccountingData();
    if (accountingData.revenue > 0 || accountingData.expenses > 0) {
      setInputs(prev => ({
        ...prev,
        grossRevenue: accountingData.revenue || prev.grossRevenue,
        allowableExpenses: accountingData.expenses || prev.allowableExpenses,
      }));
      setError(null);
    } else {
      setError("No data found in Accounting Studio. Please record transactions first.");
    }
    refreshSuggestions();
  };

  // Apply quick-fill preset
  const handleQuickFillPreset = (presetId: string) => {
    const grossIncome = inputs.grossRevenue || 0;
    if (grossIncome <= 0) {
      setError("Please enter gross revenue first before applying a preset.");
      return;
    }
    const newInputs = applyQuickFillPreset(inputs, presetId, grossIncome);
    setInputs(newInputs);
    refreshSuggestions();
    setError(null);
  };

  // Get applicable presets for current taxpayer type
  const applicablePresets = getApplicablePresets(profile.taxpayerType as 'freelancer' | 'company');

  // Handle AI Statement Analysis
  const handleStatementFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;

    // Validate file type
    const allowedTypes = ['application/pdf', 'text/csv', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'text/plain'];
    if (!allowedTypes.includes(file.type) && !file.name.endsWith('.csv') && !file.name.endsWith('.txt')) {
      setError("Please upload a PDF, CSV, Excel, or text file.");
      return;
    }
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        // For CSV/text files, try to parse financial data
        if (file.type === 'text/csv' || file.type === 'text/plain' || file.name.endsWith('.csv') || file.name.endsWith('.txt')) {
          // Simple parsing for common statement formats
          ...
          for (const line of lines) {
            ...
            // Look for revenue/income keywords
            if (lowerLine.includes('revenue') || lowerLine.includes('income') || lowerLine.includes('sales') || lowerLine.includes('turnover')) {
              const numbers = line.match(/[\d,]+\.?\d*/g);
              if (numbers) {
                const value = parseFloat(numbers[numbers.length - 1].replace(/,/g, ''));
                if (!isNaN(value) && value > extractedRevenue) {
                  extractedRevenue = value;
                }
              }
            }
            // Look for expense keywords
            if (lowerLine.includes('expense') || lowerLine.includes('cost') || lowerLine.includes('operating')) {
              const numbers = line.match(/[\d,]+\.?\d*/g);
              if (numbers) {
                const value = parseFloat(numbers[numbers.length - 1].replace(/,/g, ''));
                if (!isNaN(value) && value > extractedExpenses) {
                  extractedExpenses = value;
                }
              }
            }
          }
          ...
          if (extractedRevenue > 0 || extractedExpenses > 0) {
            setInputs(prev => ({
              ...prev,
              grossRevenue: extractedRevenue || prev.grossRevenue,
              allowableExpenses: extractedExpenses || prev.allowableExpenses,
            }));
            setStatementAnalysisResult(`✅ Extracted from ${file.name}: Revenue ₦${extractedRevenue.toLocaleString()}, Expenses ₦${extractedExpenses.toLocaleString()}`);
            refreshSuggestions();
          } else {
            setStatementAnalysisResult(`⚠️ Could not extract financial data from ${file.name}. Please enter values manually.`);
          }
        } else {
          // For PDF/Excel, show message about manual entry
          setStatementAnalysisResult(`📄 File "${file.name}" uploaded. For best results with PDF/Excel, please extract the key figures and enter them below.`);
        }
      ...
      if (file.type === 'text/csv' || file.type === 'text/plain' || file.name.endsWith('.csv') || file.name.endsWith('.txt')) {
        ...
        // For PDF/Excel, just acknowledge upload
        setStatementAnalysisResult(`📄 File "${file.name}" uploaded. Please enter the key figures from your statement below.`);
        setIsAnalyzingStatement(false);
      }
    } catch (err) {
      setError("Failed to analyze the file. Please try again or enter values manually.");
      setIsAnalyzingStatement(false);
    }
      ...
      <section id="main-calculator" className="max-w-4xl mx-auto w-full px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-gray-900">Tax Pro Max</h1>
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-blue-600 to-purple-600 text-white">
          ...
          <p className="text-sm text-gray-500 mt-1">AI-powered computation with automatic field suggestions based on PITA, CITA, and FRA 2023.</p>
        </div>
              ...
              <div className="px-4 py-5 sm:p-6 space-y-8">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-sm font-medium rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all shadow-sm"
                  >
                  ...
                  {applicablePresets.map((preset) => (
                    <button
                      key={preset.id}
                      type="button"
                      onClick={() => handleQuickFillPreset(preset.id)}
                      className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-all"
                  ...
                  <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl border border-purple-200 p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-purple-600">{autoFillSuggestions.length} recommendation{autoFillSuggestions.length !== 1 ? 's' : ''} based on Nigerian tax law</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                    ...
                    <div className="space-y-2">
                      {autoFillSuggestions.map((suggestion, idx) => {
                        const lawRef = suggestion.lawReference ? getLawReference(suggestion.lawReference) : null;
                        return (
                          <div
                            key={idx}
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3 ml-4">
                              ...
                              <button
                                ...
                                className="px-3 py-1 bg-purple-100 text-purple-700 text-xs font-medium rounded-lg hover:bg-purple-200 transition-colors"
                              >
                                Apply
                              </button>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                      <div>
                        ...
                        <select
                          ...
                          className="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md"
                        >
                          {Object.entries(STAMP_DUTY_RATES).map(([key, rate]) => (
                    ...
                    {stampDutyDocs.length > 0 && (
                      <div className="mt-4 space-y-2">
                        ...
                        {stampDutyDocs.map((doc, index) => {
                          const rateInfo = STAMP_DUTY_RATES[doc.documentType as keyof typeof STAMP_DUTY_RATES];
                          return (
                            <div
                              key={index}
                              className="flex items-center justify-between bg-white p-3 rounded-lg border border-gray-200 shadow-sm"
                            >
                              <div className="flex-1">
                                <div className="text-sm font-medium text-gray-900">
                                  {rateInfo?.label || doc.documentType}
                                </div>
                                <div className="text-xs text-gray-500 mt-1">
          ...
          <div className="mt-12 space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs text-gray-500 font-medium">
                      <span>Reference ID: {result.taxRuleMetadata?.version || 'QuantumLedger-v2'}</span>
                      <span className="opacity-30">•</span>
                      <span>{new Date().toLocaleDateString('en-NG', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">Data processed through the Quantum Ledger engine is encrypted and compliant with NDPR.</p>
                  </div>
            ...
            <div className="mt-12 text-center max-w-2xl mx-auto px-6">
              ...
              <p className="text-[11px] leading-relaxed text-gray-400 font-medium italic">
                ...
                Data processed through the Quantum Ledger engine is encrypted and compliant with NDPR.
