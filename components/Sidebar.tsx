    ...
    <>
      ...
      >
        ...
        <div className="px-2 py-3 space-y-2">
          <Link href="/" className="flex items-center gap-3 group px-2 py-1.5 rounded-md hover:bg-black/5 transition-colors">
            ...
            <h1 className="text-lg font-semibold tracking-tight text-slate-900">Atom Ledger</h1>
          </Link>
        </div>

        {/* Navigation Items */}
        <nav className="sidebar-nav-scrollbar flex-1 px-2 pr-1 space-y-1 overflow-y-auto mt-2">
          <p className="px-2 py-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500 opacity-90">
            ...
            return (
              ...
              >
                ...
                <span className={`w-5 h-5 flex items-center justify-center transition-opacity ${isActive ? 'opacity-100' : 'opacity-70 group-hover:opacity-100'}`}>
                  {isNavigating ? (
                    <div className="w-3.5 h-3.5 rounded-full bg-slate-300 animate-pulse" />
                  ) : (
